...
## 17. Importing from VS Code to Databricks

The repository root contains `databricks.yml`, so the project is already
inside the Databricks Asset Bundle workspace.

1. Open the `German_electricity_project` root folder in VS Code.
2. Install and open the Databricks extension.
3. Sign in to the workspace configured in `databricks.yml`.
4. Validate or deploy the bundle using the Databricks extension commands.
5. Confirm that the numbered project folders appear in the workspace.
6. Run the setup notebook before creating the Workflow tasks.

Every Python notebook starts with `# Databricks notebook source`, and notebook
cells are separated with `# COMMAND ----------`. Databricks therefore imports
them as notebooks rather than ordinary Python scripts.

## 18. Design decisions for an interview

Here are concise explanations for the main choices:

- **Why monthly batches?** They provide a simple restart boundary and make
  historical progress visible.
- **Why a control table?** It records completed work so the pipeline can find
  the next month and resume after failures.
- **Why keep Bronze raw?** Cleaning rules can change; Bronze preserves source
  values so Silver can be rebuilt.
- **Why use Delta MERGE?** It prevents duplicate hourly records when a batch is
  rerun.
- **Why keep nulls in Silver?** Missing reporting coverage can be meaningful;
  deleting every null row would remove valid data.
- **Why rebuild Gold?** The dataset is small, making a complete refresh simpler
  and easier to maintain than incremental aggregate logic.
- **Why four orchestration notebooks?** Each step has one responsibility and
  maps clearly to a Databricks Workflow task.

## 19. Before the first production-style run

Check the exact source column names used by the Gold load summary. Source
datasets can change naming conventions between versions. Also test one monthly
batch end to end before scheduling the remaining history. This validates the
volume path, permissions, timestamp formats, column names and table creation in
the actual Databricks workspace.

## 20. SQL analytics notebook

After the pipeline has created the Gold tables, run
`06_analytics/01_energy_analytics.sql`. It contains ten beginner-friendly SQL
analyses:

1. Recent daily load values.
2. Monthly electricity load trends.
3. Monthly load forecast accuracy.
4. Daily renewable-share trends.
5. The ten days with the highest renewable share.
6. Monthly solar and wind generation.
7. Solar-versus-wind contribution percentages.
8. Yearly renewable progress.
9. Seasonal electricity demand.
10. A combined monthly summary suitable for a dashboard.

The analytics notebook reads only the Gold layer. This is intentional:
analysts should normally query stable, business-ready tables instead of
repeating cleaning logic against Bronze or Silver. Each query has a Markdown
introduction explaining its purpose and can be converted into a Databricks
visualization after execution.
